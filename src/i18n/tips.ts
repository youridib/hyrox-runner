...
export const TIPS: Record<Language, Record<EffectiveType, string[]>> = {
  en: {
    ...
    compromised: [
      ...
      'Set the next station up before you start the run into it, then count the transition in your time. Eight transitions are 2:16 of free time.',
      'Log the 1 km split afterwards - it is what calibrates your target pace.',
    ],
    easy: [
      'Genuinely easy: conversational the whole way, no exceptions for feeling good.',
      'This is the volume that raises VO2max, which is the strongest correlate of finish time.',
      'In a heavy lifting week, bike or SkiErg instead - running carries more eccentric load.',
      'If you are tempted to push it, you are taking intensity from the sessions that need it.',
    ],
    timeTrial: [
      'Fresh legs first: this is a measurement, so treat a tired one as no measurement at all.',
      'Even pacing beats a fast start - you want your best time, not your best first lap.',
      'Same route, same conditions each time, so the trend means something.',
      'Enter the result in Settings afterwards: every pace in the plan moves with it.',
    ],
    hyrox: [
      'This is a strength day, not a run day. Keep running load off it.',
      'If a quality run lands on the same day, run first and lift at least 6 h later - 3 h is the minimum for AMPK to settle.',
  ...
  nl: {
    ...
    compromised: [
      ...
      'Zet het volgende station klaar voordat je de loop in gaat en tel de overgang mee in je tijd. Acht overgangen zijn 2:16 gratis tijd.',
      'Noteer daarna de 1 km split - die ijkt je doeltempo.',
    ],
    easy: [
      'Echt rustig: de hele loop op praattempo, ook als het goed voelt.',
      'Dit is het volume dat je VO2max optilt, de sterkste voorspeller van je eindtijd.',
      'In een zware krachtweek liever fietsen of SkiErg - lopen geeft meer excentrische belasting.',
      'Zin om aan te zetten? Dan haal je intensiteit weg bij de sessies die het nodig hebben.',
    ],
    timeTrial: [
      'Eerst frisse benen: dit is een meting, en een vermoeide meting is geen meting.',
      'Gelijkmatig lopen levert je beste tijd op, niet je beste eerste ronde.',
      'Zelfde route en omstandigheden, zodat de trend iets betekent.',
      'Zet het resultaat daarna in Instellingen: alle tempo’s in het plan bewegen mee.',
    ],
    hyrox: [
      'Dit is een krachtdag, geen loopdag. Houd looptraining eraf.',
      'Valt er een kwaliteitsloop op dezelfde dag: eerst lopen, minstens 6 uur later krachttraining - 3 uur is het minimum voor AMPK.',
